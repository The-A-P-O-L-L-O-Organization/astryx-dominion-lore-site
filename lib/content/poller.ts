...
import { campaigns, contentCache, sessionNotes, pageVisibility, sectionVisibility } from '@/lib/db/schema'
...
export async function pollCampaign(campaign: typeof campaigns.$inferSelect): Promise<void> {
...
async function parseContentDir(dir: string, campaignId: number, prefix = 'content/') {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      const data = {
        ...
        planetData: parsed.planetData ? JSON.stringify(parsed.planetData) : null,
        updatedAt: new Date().toISOString(),
      }
      db.insert(contentCache).values(data).onConflictDoUpdate({
        target: [contentCache.campaignId, contentCache.pagePath],
        set: data,
      }).run()

      const existingVis = db.select().from(pageVisibility)
        .where(and(eq(pageVisibility.campaignId, campaignId), eq(pageVisibility.pagePath, parsed.pagePath)))
        .get()
      if (!existingVis) {
        db.insert(pageVisibility).values({ campaignId, pagePath: parsed.pagePath, isHidden: true }).run()
      }

      for (const sectionId of parsed.sectionIds) {
        const existingSec = db.select().from(sectionVisibility)
          .where(and(
        ...
        if (!existingSec) {
          db.insert(sectionVisibility).values({ campaignId, pagePath: parsed.pagePath, sectionId, isHidden: true }).run()
        }
      }
    }
  }
...
async function parseSessionNotesDir(dir: string, campaignId: number) {
  ...
  for (const entry of entries) {
    if (entry.isFile() && entry.name.endsWith('.md')) {
      ...
      db.insert(sessionNotes).values(data).onConflictDoUpdate({
  ...
  for (const entry of entries) {
    if (entry.isDirectory()) {
      ...
      if (existsSync(markersPath)) {
        ...
        const existing = db.select().from(contentCache)
          .where(and(eq(contentCache.campaignId, campaignId), eq(contentCache.pagePath, pagePath)))
          .get()
        if (existing) {
          const pd = existing.planetData ? JSON.parse(existing.planetData) : {}
          db.update(contentCache)
            .set({ planetData: JSON.stringify({ ...pd, markers }), updatedAt: new Date().toISOString() })
            .where(and(eq(contentCache.campaignId, campaignId), eq(contentCache.pagePath, pagePath)))
            .run()
        }
      }
