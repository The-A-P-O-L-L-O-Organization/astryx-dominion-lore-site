...
import { getPageVisibility, getSectionVisibility, getHiddenSectionIds } from '@/lib/content/visibility'
...
describe('getPageVisibility', () => {
  ...
  it('returns true when page is not hidden', () => {
    db.insert(pageVisibility).values({ campaignId: 1, pagePath: 'factions', isHidden: false }).run()
    const result = getPageVisibility(1, 'factions', db)
    expect(result).toBe(true)
  })

  it('returns false when page is hidden', () => {
    db.insert(pageVisibility).values({ campaignId: 1, pagePath: 'factions', isHidden: true }).run()
    const result = getPageVisibility(1, 'factions', db)
    expect(result).toBe(false)
  })
  ...
  it('handles different campaigns independently', () => {
    db.insert(pageVisibility).values({ campaignId: 1, pagePath: 'factions', isHidden: false }).run()
    db.insert(pageVisibility).values({ campaignId: 2, pagePath: 'factions', isHidden: true }).run()
...
describe('getSectionVisibility', () => {
  ...
  it('returns true when section is not hidden', () => {
    db.insert(sectionVisibility).values({ campaignId: 1, pagePath: 'factions', sectionId: 'history', isHidden: false }).run()
    const result = getSectionVisibility(1, 'factions', 'history', db)
    expect(result).toBe(true)
  })

  it('returns false when section is hidden', () => {
    db.insert(sectionVisibility).values({ campaignId: 1, pagePath: 'factions', sectionId: 'history', isHidden: true }).run()
    const result = getSectionVisibility(1, 'factions', 'history', db)
    expect(result).toBe(false)
  })
  ...
  it('distinguishes sections within the same page', () => {
    db.insert(sectionVisibility).values({ campaignId: 1, pagePath: 'factions', sectionId: 'history', isHidden: false }).run()
    db.insert(sectionVisibility).values({ campaignId: 1, pagePath: 'factions', sectionId: 'leadership', isHidden: true }).run()
...
describe('getHiddenSectionIds', () => {
  ...
  it('returns only hidden section IDs', () => {
    db.insert(sectionVisibility).values({ campaignId: 1, pagePath: 'factions', sectionId: 'history', isHidden: false }).run()
    db.insert(sectionVisibility).values({ campaignId: 1, pagePath: 'factions', sectionId: 'secrets', isHidden: true }).run()
    db.insert(sectionVisibility).values({ campaignId: 1, pagePath: 'factions', sectionId: 'leadership', isHidden: true }).run()
    const result = getHiddenSectionIds(1, 'factions', db)
  ...
  it('does not return hidden sections from other pages', () => {
    db.insert(sectionVisibility).values({ campaignId: 1, pagePath: 'factions', sectionId: 'secrets', isHidden: true }).run()
    db.insert(sectionVisibility).values({ campaignId: 1, pagePath: 'planets', sectionId: 'secrets', isHidden: true }).run()
    const result = getHiddenSectionIds(1, 'factions', db)
