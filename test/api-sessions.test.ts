...
function seedNotes() {
  db.insert(sessionNotes).values({ campaignId: 1, slug: 'session-001', title: 'Session One', contentMd: 'Notes', isDmOnly: false }).run()
  db.insert(sessionNotes).values({ campaignId: 1, slug: 'session-002', title: 'Session Two', contentMd: 'Secret DM notes', isDmOnly: true }).run()
}
...
describe('GET /api/sessions', () => {
  ...
  it('returns 401 when unauthorized', async () => {
    ...
    const res = await GET(new Request('http://localhost/api/sessions?campaignId=1'))
    expect(res.status).toBe(401)
  })

  it('returns 400 without campaignId or characterId', async () => {
    const auth = await import('@/lib/auth')
    vi.mocked(auth.requireAuth).mockResolvedValue({ user: { id: 5, username: 'player1', role: 'player' } })
  ...
  it('returns sessions for a campaign (admin)', async () => {
    const auth = await import('@/lib/auth')
    vi.mocked(auth.requireAuth).mockResolvedValue({ user: { id: 1, username: 'admin', role: 'admin' } })
    seedNotes()
    const res = await GET(new Request('http://localhost/api/sessions?campaignId=1'))
  ...
  it('returns 400 for invalid campaignId', async () => {
    const auth = await import('@/lib/auth')
    vi.mocked(auth.requireAuth).mockResolvedValue({ user: { id: 1, username: 'admin', role: 'admin' } })
    const res = await GET(new Request('http://localhost/api/sessions?campaignId=abc'))
    expect(res.status).toBe(400)
  })
  ...
  it('returns only non-DM-only sessions for a character (player view)', async () => {
    const auth = await import('@/lib/auth')
    vi.mocked(auth.requireAuth).mockResolvedValue({ user: { id: 5, username: 'player1', role: 'player' } })
    db.insert(characters).values({ id: 10, userId: 5, campaignId: 1, name: 'Hero', isApproved: true }).run()
    seedNotes()
    const res = await GET(new Request('http://localhost/api/sessions?characterId=10'))
    expect(res.status).toBe(200)
    const body = await res.json()
  ...
  it('shows all sessions for admin viewing via characterId', async () => {
    const auth = await import('@/lib/auth')
    vi.mocked(auth.requireAuth).mockResolvedValue({ user: { id: 1, username: 'admin', role: 'admin' } })
    db.insert(characters).values({ id: 11, userId: 1, campaignId: 1, name: 'Hero', isApproved: true }).run()
    seedNotes()
    const res = await GET(new Request('http://localhost/api/sessions?characterId=11'))
    expect(res.status).toBe(200)
    const body = await res.json()
    expect(body).toHaveLength(2)
  })
  ...
  it('returns 400 for invalid characterId', async () => {
    const auth = await import('@/lib/auth')
    vi.mocked(auth.requireAuth).mockResolvedValue({ user: { id: 5, username: 'player1', role: 'player' } })
    const res = await GET(new Request('http://localhost/api/sessions?characterId=abc'))
    expect(res.status).toBe(400)
  })
  ...
  it('returns 404 when character not owned by user', async () => {
    const auth = await import('@/lib/auth')
    vi.mocked(auth.requireAuth).mockResolvedValue({ user: { id: 5, username: 'player1', role: 'player' } })
    db.insert(characters).values({ id: 20, userId: 999, campaignId: 1, name: 'Not Mine', isApproved: true }).run()
    const res = await GET(new Request('http://localhost/api/sessions?characterId=20'))
  ...
  it('returns 404 when character does not exist', async () => {
    const auth = await import('@/lib/auth')
    vi.mocked(auth.requireAuth).mockResolvedValue({ user: { id: 5, username: 'player1', role: 'player' } })
    const res = await GET(new Request('http://localhost/api/sessions?characterId=999'))
    expect(res.status).toBe(404)
  })
})
