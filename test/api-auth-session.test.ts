...
describe('GET /api/auth/session', () => {
  ...
  it('returns 200 when valid session exists', async () => {
    db.insert(schema.users).values({ username: 'testuser', passwordHash: 'hash', role: 'player' }).run()
    db.insert(schema.sessions).values({ id: 'valid-session', userId: 1, expiresAt: new Date(Date.now() + 86400000).toISOString() }).run()
  ...
  it('returns user null when session expired', async () => {
    db.insert(schema.users).values({ username: 'testuser', passwordHash: 'hash', role: 'player' }).run()
    db.insert(schema.sessions).values({ id: 'expired-session', userId: 1, expiresAt: new Date(Date.now() - 86400000).toISOString() }).run()
