...
describe('GET /api/campaigns', () => {
  beforeEach(() => { db.delete(schema.campaigns).run(); vi.clearAllMocks() })
...
describe('PUT /api/campaigns', () => {
  ...
  it('updates a campaign', async () => {
    ...
    const req = new Request('http://localhost/api/campaigns', {
      ...
      body: JSON.stringify({ id: 1, name: 'Updated', loreRepoUrl: 'x', description: '', theme: 'fantasy', isHidden: false, starMapConfig: '{}' }),
    })
...
describe('DELETE /api/campaigns', () => {
  ...
  it('deletes a campaign', async () => {
    db.insert(schema.campaigns).values({ name: 'Delete me', loreRepoUrl: 'x' }).run()
    const req = new Request('http://localhost/api/campaigns', {
