...
vi.mock('@/lib/auth', () => ({
  requireAdmin: vi.fn(() => Promise.resolve({ id: 1, username: 'admin', role: 'admin' })),
}))
...
describe('GET /api/visibility', () => {
  ...
  it('returns pages and sections for a campaign', async () => {
    db.insert(schema.pageVisibility).values({ campaignId: 1, pagePath: 'factions', isHidden: false }).run()
    db.insert(schema.pageVisibility).values({ campaignId: 1, pagePath: 'planets', isHidden: true }).run()
    db.insert(schema.sectionVisibility).values({ campaignId: 1, pagePath: 'factions', sectionId: 'history', isHidden: false }).run()
    db.insert(schema.sectionVisibility).values({ campaignId: 1, pagePath: 'factions', sectionId: 'secrets', isHidden: true }).run()
  ...
  it('filters by campaignId', async () => {
    db.insert(schema.pageVisibility).values({ campaignId: 1, pagePath: 'pg1' }).run()
    db.insert(schema.pageVisibility).values({ campaignId: 2, pagePath: 'pg2' }).run()
    const req = new Request('http://localhost/api/visibility?campaignId=1')
    const res = await GET(req)
    const body = await res.json()
...
describe('PUT /api/visibility', () => {
  ...
  it('toggles page visibility', async () => {
    db.insert(schema.pageVisibility).values({ campaignId: 1, pagePath: 'factions', isHidden: true }).run()
    const req = new Request('http://localhost/api/visibility', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ type: 'page', campaignId: 1, pagePath: 'factions', isHidden: false }),
    })
  ...
  it('toggles section visibility', async () => {
    ...
    const req = new Request('http://localhost/api/visibility', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ type: 'section', campaignId: 1, pagePath: 'factions', sectionId: 'secrets', isHidden: false }),
    })
    const res = await PUT(req)
    expect(res.status).toBe(200)
