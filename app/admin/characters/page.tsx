...
interface Character {
  id: number; userId: number; campaignId: number; name: string; info: string; isApproved: boolean | number; createdAt: string
}
        ...
        <Card key={c.id} className="mb-3">
          ...
          <CardContent>
            ...
            <div className="flex gap-2">
              <Button size="sm" onClick={() => approve(c.id, true)}>Approve</Button>
              <Button size="sm" variant="destructive" onClick={() => approve(c.id, false)}>Reject</Button>
            </div>
          </CardContent>
        </Card>
      ))}

      <h2 className="text-xl font-semibold mb-3 mt-8">Approved ({approved.length})</h2>
      {approved.map(c => (
