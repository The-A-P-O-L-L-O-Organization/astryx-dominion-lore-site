...
export async function GET(request: Request) {
  try {
    ...
    const pages = db.select().from(pageVisibility)
      .where(eq(pageVisibility.campaignId, Number(campaignId)))
      .all()

    const sections = db.select().from(sectionVisibility)
...
export async function PUT(request: Request) {
  try {
    ...
    if (body.type === 'page') {
      db.update(pageVisibility).set({ isHidden: body.isHidden })
        .where(and(eq(pageVisibility.campaignId, body.campaignId), eq(pageVisibility.pagePath, body.pagePath)))
        .run()
    } else if (body.type === 'section') {
      db.update(sectionVisibility).set({ isHidden: body.isHidden })
        .where(and(
