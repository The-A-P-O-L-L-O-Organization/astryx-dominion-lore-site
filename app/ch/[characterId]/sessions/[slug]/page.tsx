...
export default async function SessionNotePage({ params }: { params: Promise<{ characterId: string; slug: string }> }) {
  ...
  const note = db.select().from(sessionNotes)
    .where(and(eq(sessionNotes.campaignId, character.campaignId), eq(sessionNotes.slug, slug)))
    .get()
  ...
  return (
    <div className="min-h-screen p-8 max-w-4xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        <Link href={`/ch/${characterId}/sessions`}>
          <Button variant="ghost" size="icon"><ArrowLeft className="h-4 w-4" /></Button>
        </Link>
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <h1 className="text-3xl font-bold">{note.title}</h1>
            {note.isDmOnly ? <Badge variant="secondary">DM&apos;s Notes</Badge> : <Badge>Session Log</Badge>}
          </div>
          <p className="text-sm text-muted-foreground">
            {new Date(note.createdAt).toLocaleDateString()} &middot; {note.source === 'git' ? 'From lore repo' : 'Written in-app'}
          </p>
        </div>
      </div>
