...
type CelestialBody = {
  type: string; name: string; color: string; orbit_radius?: number; orbit_speed?: number
  terrain_type?: string; star_type?: string; belt_width?: number; belt_density?: number
  parent_id?: string; markers?: any[]; pagePath: string
}

export function StarmapClient({ config, bodies, campaignName }: {
  config: any
  ...
  const systems = (config.systems || []) as Array<{ id: string; name: string; x: number; y: number; z: number; star: { color: string; size: number; star_type: string } }>
  ...
  if (view === 'galaxy') {
    return (
      <div className="w-full h-screen">
        <GalaxyView systems={systems} hyperlanes={config.hyperlanes || []} onSelectSystem={handleSelectSystem} />
      </div>
    )
  }
      ...
      <div className="w-full h-screen flex">
        ...
        <aside className="w-96 border-l border-border p-4 overflow-y-auto">
          ...
          <p className="text-sm text-muted-foreground">
            Planet surface view. Markers appear as red pins. Click and drag to rotate. Scroll to zoom.
          </p>
        </aside>
      </div>
    )
