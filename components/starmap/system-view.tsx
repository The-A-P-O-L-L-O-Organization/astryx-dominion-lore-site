...
function OrbitingBody({ body, index, onSelect }: { body: CelestialBody; index: number; onSelect: (path: string) => void }) {
  ...
  const size = body.type === 'moon' ? 0.15 : body.type === 'station' ? 0.12 : body.type === 'asteroid_belt' ? 0.05 : 0.3
  ...
  return (
    <group>
      <line>
        <bufferGeometry>
          <bufferAttribute
            ...
            args={[
              new Float32Array(Array.from({ length: 65 }, (_, i) => {
      ...
      <Text position={[radius + 0.5, 0.3, 0]} fontSize={0.15} color="white" anchorX="left">
        {body.name}
      </Text>
    </group>
  )
...
export function SystemView({ starColor, starSize, bodies, onSelectPlanet, onBack }: SystemViewProps) {
  return (
    <div className="relative w-full h-full">
      ...
      <Canvas camera={{ position: [0, 5, 8], fov: 50 }}>
        ...
        {bodies.map((body, i) => (
          <OrbitingBody key={body.pagePath} body={body} index={i} onSelect={onSelectPlanet} />
        ))}
